...
function handleTextureLoaded(texture) {
  ...
  // If the image has an alpha channel, enable premultiplication so blending looks correct
  gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL, 1);
  // gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, 1); // use it to flip Y if needed
  // Use RGBA so PNG alpha channel is preserved for blending
  gl.texImage2D(
...
function initCubeMap() {
  let cubeMapPath = "./texture_and_other_files/Field/";
  faceImages = [
    {
